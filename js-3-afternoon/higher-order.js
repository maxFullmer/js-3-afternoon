...
//Code Here
let postTaxPrices = prices.map((price) => {return price * 1.07});
...
//Code Here
let totalPopulation = populations.reduce((runningTotal, populationCount) => {return runningTotal + populationCount});
...
//Code Here
let myStrongest = monstersInYourPocket.filter((monster,i) => {return monster["CP"] > 200});
...
let orderTotals = orders.map((cost) => {return (cost["price"] + cost["price"]*cost["tax"])});
...
let bobsTotal = purchases.filter((owners) => {return owners["owner"] === "Bob"}).reduce((runningTotal, prices) => 

{
  console.log(runningTotal);
  console.log(prices["price"]);
  return runningTotal + prices["price"]
},0);
